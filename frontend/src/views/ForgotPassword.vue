  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50">
    <div class="flex w-full max-w-4xl bg-white rounded-xl shadow-lg overflow-hidden">
      ...
      <div class="w-1/2 p-8 flex flex-col justify-center">
        ...
        <h3 class="text-2xl font-semibold text-gray-800 mb-6">Mot de passe oublié</h3>
        <form @submit.prevent="handleForgotPassword">
          ...
          <button type="submit" :disabled="loading" class="w-full bg-orange-600 text-white font-semibold py-2 rounded-lg hover:bg-orange-700 transition flex items-center justify-center">
            ...
            {{ loading ? 'Envoi...' : 'Recevoir un code par mail' }}
          </button>
        </form>
      </div>
    </div>
...
const handleForgotPassword = async () => {
  ...
  try {
    ...
    message.value = 'Un email de réinitialisation a été envoyé si l\'adresse existe.'
  } catch (err) {
    error.value = err.message || 'Erreur lors de l\'envoi du mail.'
  } finally {
    loading.value = false
  }
}
